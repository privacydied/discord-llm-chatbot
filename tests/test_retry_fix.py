...
from bot.retry_utils import VISION_RETRY_CONFIG, is_retryable_error


def test_retry_logic() -> None:
    """Test that 502 errors are properly detected as retryable."""
    # Test 1: 502 error should be retryable
    error_502 = APIError("Error code: 502 - {'error': {'message': 'Provider returned error', 'code': 502}}")
    assert is_retryable_error(error_502, VISION_RETRY_CONFIG), "502 error should be retryable"
    ...
    assert is_retryable_error(api_error_502, VISION_RETRY_CONFIG), "Actual 502 error should be retryable"

    # Test 3: "Provider returned error" pattern should be retryable
    provider_error = APIError("Provider returned error from Chutes")
    assert is_retryable_error(provider_error, VISION_RETRY_CONFIG), "Provider error should be retryable"
