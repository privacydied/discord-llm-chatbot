...
# Import the fixed KokoroDirect class via importlib to avoid E402
...
def test_tts_pipeline():
    ...
    try:
        ...
        if (
            ...
            logger.info(
                f"Success! Audio generated at: {result_path} ({result_path.stat().st_size} bytes)"
            )
            return True
        else:
            logger.error(
                f"Failed: Result is not a valid Path or file is empty. Result: {result_path}"
            )
            return False
