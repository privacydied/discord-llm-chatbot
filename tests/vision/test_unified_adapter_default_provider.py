...
@pytest.mark.parametrize(
    ("task", "kwargs"),
    [
        (VisionTask.IMAGE_TO_IMAGE, {"input_image_data": b"fake-image-bytes"}),
        (VisionTask.VIDEO_GENERATION, {}),
    ],
)
async def test_default_provider_skipped_when_it_cant_do_the_task(adapter, task, kwargs) -> None:
    """nvidia has neither IMAGE_TO_IMAGE nor VIDEO_GENERATION capability (see
    tests/vision/test_nvidia_plugin_image_edit.py for why IMAGE_TO_IMAGE was
    tried and reverted) - promoting it to the front must NOT break either;
    it should be silently skipped and the next capable provider used instead.
    """
    request = VisionRequest(
        task=task,
