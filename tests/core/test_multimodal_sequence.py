...
class TestMultimodalSequence:
    ...
    async def test_mixed_attachments_and_urls(self, router, mock_message, mock_attachment, mock_pdf_attachment):
        ...
        with patch.object(router, '_handle_video_url', new_callable=AsyncMock) as mock_video, \
             patch.object(router, '_handle_general_url', new_callable=AsyncMock) as mock_url, \
             ...
             patch.object(router, '_flow_process_text', new_callable=AsyncMock) as mock_text_flow:
            ...
            await router._process_multimodal_message_internal(mock_message, "test context")
    ...
    async def test_no_inputs_text_only(self, router, mock_message):
        ...
        with patch.object(router, '_invoke_text_flow', new_callable=AsyncMock) as mock_text_flow:
            await router._process_multimodal_message_internal(mock_message, "test context")
    ...
    async def test_url_stripping_from_text(self, router, mock_message):
        ...
        with patch.object(router, '_handle_general_url', new_callable=AsyncMock) as mock_url, \
             patch.object(router, '_flow_process_text', new_callable=AsyncMock) as mock_text_flow:
    ...
    async def test_sequential_processing_order(self, router, mock_message):
        ...
        with patch.object(router, '_handle_general_url', side_effect=track_url_call), \
