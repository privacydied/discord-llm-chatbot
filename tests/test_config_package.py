...
class TestGetConfigImportable:
    ...
    def test_get_config_exists_or_skipped(self):
...
class TestLoadSystemPrompts:
    ...
    def test_load_system_prompts_returns_dict(self):
        from bot.config import load_system_prompts

        result = load_system_prompts()
        assert isinstance(result, dict)

    def test_load_system_prompts_has_expected_keys(self):
        from bot.config import load_system_prompts
...
class TestCheckVenvActivation:
    ...
    def test_check_venv_activation_no_raise(self):
...
class TestValidateRequiredEnv:
    ...
    def test_validate_required_env_raises_when_missing(self):
    ...
    def test_validate_required_env_passes_when_set(self, monkeypatch):
...
class TestGetVlModelLadder:
    ...
    def test_get_vl_model_ladder_returns_list(self):
        from bot.config import get_vl_model_ladder

        result = get_vl_model_ladder()
        assert isinstance(result, list)

    def test_get_vl_model_ladder_non_empty_default(self, monkeypatch):
    ...
    def test_get_vl_model_ladder_respects_env(self, monkeypatch):
...
class TestLoadConfigKeys:
    ...
    def test_expected_key_present(self, config, key):
        assert key in config, f"load_config() missing expected key: {key}"

    def test_key_values_are_reasonable(self, config):
