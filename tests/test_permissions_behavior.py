...
@pytest.mark.asyncio
async def test_is_admin_owner_allowed():
...
@pytest.mark.asyncio
async def test_is_admin_owner_id_allowed():
...
@pytest.mark.asyncio
async def test_is_non_admin_denied():
...
@pytest.mark.asyncio
async def test_is_member_admin_allowed():
...
@pytest.mark.asyncio
async def test_is_non_admin_member_denied():
...
async def test_configured_admin_ids_allowed(mock_ids):
...
@pytest.mark.asyncio
async def test_prefix_decorator_blocks_non_admin():
...
@pytest.mark.asyncio
async def test_prefix_decorator_allows_admin():
...
async def test_slash_decorator_blocks_non_admin_ephemeral(mock_is_admin):
...
@pytest.mark.asyncio
@patch("bot.core.permissions.is_admin_user")
async def test_slash_decorator_allows_admin(mock_is_admin):
    mock_is_admin.return_value = True

    handler_reached = []

    @admin_only_slash()
    async def my_slash(interaction):
...
@pytest.mark.asyncio
async def test_check_admin_async_returns_true_for_admin():
    bot = _make_bot(owner_ids=[100])
    user = _make_user(user_id=100)
...
@pytest.mark.asyncio
async def test_check_admin_async_returns_false_for_non_admin():
    bot = _make_bot(owner_ids=[100])
    user = _make_user(user_id=999)
    result = await check_admin_async(user, bot)
    assert result is False
...
@pytest.mark.asyncio
async def test_check_admin_async_denial_on_prefix_no_ephemeral():
...
@pytest.mark.asyncio
async def test_check_admin_async_denial_on_interaction_ephemeral():
...
@pytest.mark.asyncio
async def test_check_admin_async_no_denial_without_channel():
...
@pytest.mark.asyncio
async def test_dm_user_non_admin_denied():
