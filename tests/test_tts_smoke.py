...
class TTSSmokeTest(unittest.TestCase):
    ...
    def test_generate_audio(self) -> None:
        ...
        try:
            ...
            assert isinstance(result, Path), f"Expected Path object, got {type(result)} instead"

            # Verify that the file exists
            assert result.exists(), f"Output file {result} does not exist"

            # Verify that the file has content (should be > 10,000 bytes for "ping")
            file_size = result.stat().st_size
            assert file_size > 10000, f"File size ({file_size} bytes) is too small"

            # Verify audio duration is at least 1 second
            audio_info = sf.info(result)
            duration = audio_info.duration
            assert duration >= 1.0, f"Audio duration ({duration:.2f}s) is too short"
