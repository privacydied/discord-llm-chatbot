...
async def test_unroll_logging_ok(monkeypatch, caplog) -> None:
    ...
    async def fake_unroll(url: str, *, timeout_s: float, max_tweets: int, max_chars: int):
        ctx = SimpleNamespace(joined_text="Thread joined text", tweet_count=3, canonical_url=url)
        return ctx, None
...
async def test_unroll_logging_fallback(monkeypatch, caplog) -> None:
    ...
    assert any(isinstance(getattr(rec, "detail", None), dict) and getattr(rec, "detail", {}).get("reason") == "dom_mismatch" for rec in fallback_recs)
