...
def test_model_aliases():
    ...
    for (
        ...
        assert selection.provider == expected_provider, (
            f"Wrong provider for {alias}: {selection.provider} != {expected_provider}"
        )
        assert selection.endpoint == expected_endpoint, (
            f"Wrong endpoint for {alias}: {selection.endpoint} != {expected_endpoint}"
        )
        assert selection.model_hint == expected_model, (
            f"Wrong model for {alias}: {selection.model_hint} != {expected_model}"
        )
        assert selection.supports_advanced == expected_advanced, (
            f"Wrong advanced support for {alias}: {selection.supports_advanced} != {expected_advanced}"
        )
...
def test_qwen_parameter_normalization():
    ...
    def normalize_size_for_qwen(width, height):
        ...
        if width > max_w or height > max_h:
            ...
            warnings.append(
                f"Size downscaled from {original_w}x{original_h} to {width}x{height} for qwen-image-txt2img limits"
            )
    ...
    for case in test_cases:
        ...
        assert payload["size"] == case["expected_size"], (
            f"Wrong size: {payload['size']} != {case['expected_size']}"
        )
        assert len(warnings) == case["expected_warnings"], (
            f"Wrong warning count: {len(warnings)} != {case['expected_warnings']}"
        )
...
def test_sdxl_parameter_support():
    ...
    def build_sdxl_payload(
        prompt, negative_prompt, width, height, steps, guidance_scale, seed, safety_mode
    ):
