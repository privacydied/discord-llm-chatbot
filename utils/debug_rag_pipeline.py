...
_ua_hs = importlib.import_module("bot.rag.hybrid_search")
get_hybrid_search = getattr(_ua_hs, "get_hybrid_search")
_ua_chrom = importlib.import_module("bot.rag.chroma_backend")
ChromaRAGBackend = getattr(_ua_chrom, "ChromaRAGBackend")
_ua_doc = importlib.import_module("bot.rag.document_parsers")
document_parser_factory = getattr(_ua_doc, "document_parser_factory")
_ua_chunk = importlib.import_module("bot.rag.text_chunker")
create_chunker = getattr(_ua_chunk, "create_chunker")
_ua_vs = importlib.import_module("bot.rag.vector_schema")
HybridSearchConfig = getattr(_ua_vs, "HybridSearchConfig")
_ua_embed = importlib.import_module("bot.rag.embedding_interface")
create_embedding_model = getattr(_ua_embed, "create_embedding_model")
_ua_log = importlib.import_module("bot.utils.logging")
get_logger = getattr(_ua_log, "get_logger")
...
class RAGPipelineDebugger:
    ...
    async def _audit_knowledge_base(self):
        ...
        for file_path in files:
            if file_path.is_file():
                ...
                supported = (
                    extension in document_parser_factory.get_supported_extensions()
                )
    ...
    async def _audit_document_parsing(self):
        ...
        try:
            ...
            content, metadata = await document_parser_factory.parse_document(
                gateway_file
            )
            ...
            print(
                f"🔍 Key terms found: {', '.join(found_terms) if found_terms else 'None'}"
            )
    ...
    async def _audit_chromadb_collection(self):
        ...
        try:
            ...
            backend = ChromaRAGBackend(db_path=str(self.db_path), config=self.config)
    ...
    async def _audit_embedding_generation(self):
        ...
        try:
            ...
            print(
                f"📊 Embedding dimension: {await embedding_model.get_embedding_dimension()}"
            )
            ...
            for query in self.test_queries:
                try:
                    ...
                    results = await search_engine._vector_search(
                        query=query, user_id=None, guild_id=None, max_results=5
                    )
    ...
    async def _debug_specific_query(self, query: str):
        ...
        try:
            ...
            try:
                ...
                if search_results.get("ids") and search_results["ids"][0]:
                    ...
                    for i in range(min(3, len(search_results["ids"][0]))):
                        ...
                        distance = (
                            search_results.get("distances", [[]])[0][i]
                            if search_results.get("distances")
                            else "N/A"
                        )
                        document = (
                            search_results.get("documents", [[]])[0][i]
                            if search_results.get("documents")
                            else ""
                        )
                        metadata = (
                            search_results.get("metadatas", [[]])[0][i]
            ...
            try:
                backend_results = await backend.search(query=query, n_results=5)
