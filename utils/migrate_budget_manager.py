...
def backup_data(data_dir: Path) -> Path:
    ...
    backup_dir = (
        data_dir.parent / f"vision_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    )
        ...
        for user_id, budget in data.items():
            ...
            for field in money_fields:
                if field in budget and not isinstance(budget[field], str):
                    logger.error(
                        f"User {user_id} field {field} is not a string: {type(budget[field])}"
                    )
                    return False
...
def main():
    ...
    if not verify_migration(data_dir):
        logger.error(
            f"Migration verification failed! Backup preserved at: {backup_dir}"
        )
        if backup_dir:
