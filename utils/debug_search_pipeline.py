...
_ua_chroma = importlib.import_module("bot.rag.chroma_backend")
ChromaRAGBackend = getattr(_ua_chroma, "ChromaRAGBackend")
...
async def debug_search_pipeline():
    ...
    try:
        ...
        try:
            ...
            print(
                f"   Direct search results: {len(search_results.get('ids', [[]])[0])}"
            )
        ...
        try:
            ...
            low_threshold_results = await backend_low_threshold.search(
                test_query, n_results=5
            )
