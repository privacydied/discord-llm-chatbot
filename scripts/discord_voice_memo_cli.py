...
Environment Variables:
  ...
  KOKORO_VOICES    - Path to Kokoro voice embeddings
    ...
    parser.add_argument(
        "channel_id",
        type=int,
        help="Discord channel ID to send voice memo to"
    )
    ...
    parser.add_argument(
        "--token",
        help="Discord bot token (overrides DISCORD_TOKEN env var)"
    )
    
    parser.add_argument(
        "--voice", 
        default="af_sky",
        help="Voice to use for synthesis (default: af_sky)"
    )
    ...
    parser.add_argument(
        "--model-path",
        help="Path to Kokoro ONNX model (overrides KOKORO_MODEL env)"
    )
    ...
    if not bot_token:
        logger.error("Discord bot token required. Use --token or set DISCORD_TOKEN environment variable.")
        return 1
    ...
    if not model_path:
        logger.error("Kokoro model path required. Use --model-path or set KOKORO_MODEL environment variable.")
        return 1
        
    if not voices_path:
        logger.error("Kokoro voices path required. Use --voices-path or set KOKORO_VOICES environment variable.")
        return 1
    ...
    try:
        ...
        # Generate speech
        logger.info(f"Generating speech for: '{args.text[:50]}{'...' if len(args.text) > 50 else ''}'")
