...
KOKORO_MODEL_URL = (
    "https://github.com/PromtEngineer/kokoro-onnx-tts/raw/main/kokoro-v1.0.onnx"
)
...
def download_file(url, dest_path, backup_url=None):
    ...
    try:
        ...
        if response.status_code != 200 and backup_url:
            logging.warning(
                f"Failed to download from primary URL (status {response.status_code}). Trying backup URL..."
            )
        ...
        logging.info(
            f"Successfully downloaded {dest_path} ({dest_path.stat().st_size} bytes)"
        )
        return True
...
def validate_voice_file(path):
    ...
    try:
        ...
        if len(sample_voice) < 100:
            logging.warning(
                f"Voice embedding dimension seems too small: {len(sample_voice)}"
            )

        logging.info(
            f"Voice file validated: {len(data)} voices, embedding dim={len(sample_voice)}"
        )
        return True
    except Exception as e:
...
def main():
    ...
    if not model_path.exists() or model_path.stat().st_size < 10000000:  # 10MB minimum
        ...
        print(
            f"✅ Model file already exists: {model_path} ({model_path.stat().st_size} bytes)"
        )
