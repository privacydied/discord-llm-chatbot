    ...
    try:
        ...
        try:
            ...
            if "onnxruntime-gpu" in installed_packages:
                print(
                    "[FATAL] Conflicting package 'onnxruntime-gpu' is still installed.",
                    flush=True,
                )
            else:
            ...
            print(
                "[WARN] `pkg_resources` not found, cannot check for conflicting packages.",
                flush=True,
            )
        except Exception as e:
        ...
        print(
            f"[FATAL] An unexpected error occurred during diagnostics: {e}", flush=True
        )
    finally:
