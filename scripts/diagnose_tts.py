...
def check_imports():
    ...
    try:
        ...
        logging.info(
            f"✅ kokoro_onnx imported successfully. Version: {getattr(kokoro_onnx, '__version__', 'unknown')}"
        )

        try:
            importlib.import_module("misaki.en")
            importlib.import_module("misaki.espeak")
            logging.info("✅ misaki (en, espeak) imported successfully")
        except Exception as e:
            logging.error(f"❌ Failed to import misaki submodules: {e}")
            return False
...
def check_model_files():
    ...
    if not model_file.exists():
        ...
        return False
    logging.info(
        f"✅ Found model file: {model_file} ({model_file.stat().st_size} bytes)"
    )
    ...
    if not voices_file.exists():
        ...
        return False
    logging.info(
        f"✅ Found voices file: {voices_file} ({voices_file.stat().st_size} bytes)"
    )
    ...
    try:
        ...
        if not isinstance(voices_data, dict):
            logging.warning(
                f"⚠️ voices.json is not a dictionary. Type: {type(voices_data)}"
            )
            return False
        ...
        if first_voice:
            ...
            logging.info(
                f"First voice embedding type: {type(voices_data[first_voice])}"
            )
...
def initialize_tts():
    ...
    try:
        ...
        # Initialize Kokoro
        logging.info(
            f"Initializing Kokoro with model={model_path}, voices={voices_path}"
        )
        ...
        if hasattr(engine, "voices"):
            ...
            if available_voices:
                ...
                samples, sample_rate = engine.create(
                    text, test_voice, phonemes=phonemes
                )
                logging.info(
                    f"✅ Audio generated: {len(samples)} samples at {sample_rate}Hz"
                )
            ...
            for line in env_content.splitlines():
                if line.startswith("TTS_BACKEND="):
                    ...
                    if backend != "kokoro-onnx":
                        logging.warning(
                            f"⚠️ TTS_BACKEND is not set to kokoro-onnx: {backend}"
                        )
