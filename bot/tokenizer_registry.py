...
class TokenizerRegistry:
    ...
    def __init__(self):
        ...
        if TokenizerRegistry._instance is not None:
            logger.warning(
                "TokenizerRegistry initialized more than once",
    ...
    def discover_tokenizers(self, force: bool = False) -> Dict[str, bool]:
        ...
        if self._initialized and not force:
            logger.debug(
                "Tokenizer discovery already performed, skipping",
        ...
        if diagnostics["espeak_binary"]:
            try:
                ...
                if result.returncode == 0:
                    ...
                    logger.info(
                        "espeak tokenizer is available",
                        extra={"subsys": "tts", "event": "registry.available.espeak"},
                    )
            except (subprocess.SubprocessError, FileNotFoundError) as e:
                logger.info(
                    f"espeak binary found but not working: {e}",
        ...
        if diagnostics["espeak_ng_binary"]:
            try:
                ...
                if result.returncode == 0:
                    ...
                    logger.info(
                        "espeak-ng tokenizer is available",
                        extra={
                            "subsys": "tts",
                            "event": "registry.available.espeak_ng",
                        },
                    )
            except (subprocess.SubprocessError, FileNotFoundError) as e:
                logger.info(
                    f"espeak-ng binary found but not working: {e}",
        ...
        if diagnostics["phonemizer_module"]:
            if importlib.util.find_spec("phonemizer") is not None:
                ...
                logger.info(
                    "phonemizer tokenizer is available",
                    extra={"subsys": "tts", "event": "registry.available.phonemizer"},
                )
            else:
                logger.info(
                    "phonemizer module found but import failed",
        ...
        if diagnostics["g2p_en_module"]:
            if importlib.util.find_spec("g2p_en") is not None:
                ...
                logger.info(
                    "g2p_en tokenizer is available",
                    extra={"subsys": "tts", "event": "registry.available.g2p_en"},
                )
            else:
                logger.info(
                    "g2p_en module found but import failed",
        ...
        if diagnostics["misaki_module"]:
            if importlib.util.find_spec("misaki") is not None:
                ...
                logger.info(
                    "misaki tokenizer is available",
                    extra={"subsys": "tts", "event": "registry.available.misaki"},
                )
            else:
                logger.info(
                    "misaki module found but import failed",
                    extra={"subsys": "tts", "event": "registry.unavailable.misaki"},
                )

        # Log the discovered tokenizers
        logger.info(
            f"Tokenizer discovery completed: {', '.join(sorted(self._available_tokenizers))}",
    ...
    def select_tokenizer_for_language(self, language: str) -> str:
        ...
        if len(self._available_tokenizers) < self._size_at_init:
            logger.error(
                "Tokenizer registry has been corrupted (size decreased)",
            ...
            if env_path:
                ...
                if p.exists():
                    ...
                    logger.warning(
                        ...
                        extra={
                            "subsys": "tts",
                            "event": "registry.lexicon.missing_path",
                        },
                    )
            else:
                default = (
    ...
    def is_tokenizer_warning_needed(self, language: str = None) -> bool:
        ...
        if language == "en":
            logger.debug(
                "English uses built-in IPA path, no external tokenizers required",
    ...
    def select_for_language(self, language: str, text: str) -> Decision:
        ...
        logger.debug(
            f"Using grapheme tokenization for {language}",
