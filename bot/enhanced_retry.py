...
from .config import get_vl_model_ladder, load_config
...
def _is_image_capable_model(model: str) -> bool:
    """Check if a model is known to support image input [REH][IV]."""
    if not model:
        return False
    model_lower = model.lower().strip()
    # Check exact match first
    if model_lower in _IMAGE_CAPABLE_MODELS:
        return True
    # Check without :free suffix
    base_model = model_lower.rsplit(":free", 1)[0]
    if base_model in _IMAGE_CAPABLE_MODELS:
        return True
    # Check by keyword patterns (heuristic for new models)
    vl_keywords = ("vision", "-vl-", "/vl-", "vl:", "pixtral", "gemini")
...
class EnhancedRetryManager:
    ...
    def _apply_vl_override(self, vision_from_env: bool = False) -> None:
        ...
        If vision ladder came from env, respect env order and do not reorder/filter.
        """
        if vision_from_env:
            return
        try:
    ...
    def _is_retryable_error(self, error: Exception) -> bool:
        ...
        if ("401" in error_str or "403" in error_str) and any(
            pat in error_str
            for pat in (
                "authentication",
                "unauthorized",
                ...
                try:
                    ...
                    if not self._is_retryable_error(e):
                        ...
                        if any(
                            ...
                            logger.exception(f"❌ Authentication failure for {provider_key}; aborting ladder: {type(e).__name__}: {e}")
                            total_time = time.time() - start_time
                            return RetryResult(
                                success=False,
                                error=e,
                    ...
                    if attempt < provider_config.max_attempts - 1:
                        ...
                        if (not use_retry_after) and provider_config.jitter:
                            delay *= 0.8 + random.random() * 0.4  # nosec B311
