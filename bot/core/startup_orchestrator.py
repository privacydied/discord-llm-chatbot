...
from bot.metrics import metrics, METRIC_STARTUP_TOTAL_DURATION, METRIC_STARTUP_COMPONENT_DURATION, METRIC_STARTUP_PARALLEL_GROUPS, METRIC_COMPONENT_INIT_SUCCESS, METRIC_COMPONENT_INIT_FAILURE, METRIC_COMPONENT_LAST_INIT_TIMESTAMP, METRIC_DEGRADED_MODE
...
class StartupOrchestrator:
    ...
    def add_component(self, spec: ComponentSpec) -> None:
        ...
        self.logger.debug(f"🔧 Added component: {spec.name} (deps: {spec.dependencies})", 
    ...
    async def _initialize_component(self, spec: ComponentSpec) -> ComponentResult:
        ...
        for attempt in range(spec.retry_count + 1):
            try:
                self.logger.info(f"🔧 Initializing {spec.name} (attempt {attempt + 1}/{spec.retry_count + 1})", 
                               extra={"subsys": "startup"})
                ...
                metrics.gauge(METRIC_COMPONENT_LAST_INIT_TIMESTAMP, time.time(), {"component": spec.name})
                
                self.logger.info(f"✅ {spec.name} initialized successfully ({duration_ms:.1f}ms)", 
                               extra={"subsys": "startup"})
                ...
                self.logger.warning(f"⏱️ {spec.name} initialization timeout (attempt {attempt + 1})", 
                                  extra={"subsys": "startup"})
                
            except spec.expected_exceptions as e:
                last_error = str(e)
                error_class = self._classify_error(e)
                self.logger.warning(f"⚠️ {spec.name} initialization failed: {e} (attempt {attempt + 1})", 
                                  extra={"subsys": "startup"})
                
            except Exception as e:
                last_error = str(e)
                error_class = ErrorClass.PROGRAMMING_ERROR
                self.logger.error(f"❌ {spec.name} unexpected error: {e} (attempt {attempt + 1})", 
                                exc_info=True, extra={"subsys": "startup"})
        ...
        if spec.fallback:
            try:
                self.logger.info(f"🔄 Attempting fallback for {spec.name}", 
                               extra={"subsys": "startup"})
                ...
                self.logger.warning(f"🔄 {spec.name} using fallback after {spec.retry_count + 1} attempts", 
                                  extra={"subsys": "startup"})
                ...
                self.logger.error(f"❌ {spec.name} fallback also failed: {fallback_error}", 
                                exc_info=True, extra={"subsys": "startup"})
    ...
    async def execute(self) -> Dict[str, ComponentResult]:
        ...
        try:
            ...
            self.logger.info(f"🚀 Starting parallel initialization: {len(groups)} groups, {len(self.components)} components", 
                           extra={"subsys": "startup"})
            ...
            for group_idx, group in enumerate(groups):
                ...
                self.logger.info(f"🔧 Initializing group {group_idx + 1}/{len(groups)}: {group}", 
                               extra={"subsys": "startup"})
                ...
                for result in group_results:
                    if isinstance(result, ComponentResult):
                        ...
                        metrics.observe(METRIC_STARTUP_COMPONENT_DURATION, result.duration_ms / 1000, 
                                      {"component": result.name, "status": result.status.value})
                    else:
                        self.logger.error(f"Unexpected result type: {result}", extra={"subsys": "startup"})
                
                group_duration = (time.time() - group_start) * 1000
                self.logger.info(f"✅ Group {group_idx + 1} completed in {group_duration:.1f}ms", 
                               extra={"subsys": "startup"})
                
                # Check for fatal failures before continuing
                fatal_failures = [
                    r for r in group_results 
                    if isinstance(r, ComponentResult) and r.status == ComponentStatus.FAILED 
            ...
            total_duration = time.time() - startup_start
            self.logger.error(f"💥 Startup orchestration failed after {total_duration:.2f}s: {e}", 
                            exc_info=True, extra={"subsys": "startup"})
            raise
