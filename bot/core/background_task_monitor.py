...
from bot.metrics import metrics, METRIC_BACKGROUND_HEARTBEAT, METRIC_BACKGROUND_LAST_HEARTBEAT, METRIC_BACKGROUND_CONSECUTIVE_ERRORS, METRIC_BACKGROUND_STALENESS_SECONDS
...
class HeartbeatWrapper:
    ...
    async def __aenter__(self):
        ...
        self.logger.debug(f"💓 Starting heartbeat wrapper for {self.task_name}", 
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        ...
        if exc_type:
            self.logger.warning(f"💓 Heartbeat wrapper exiting due to {exc_type.__name__}: {exc_val}", 
                               extra={"subsys": "background_task"})
        else:
            self.logger.debug(f"💓 Heartbeat wrapper completed for {self.task_name}", 
                             extra={"subsys": "background_task"})
...
class BackgroundTaskMonitor:
    ...
    def register_task(self, config: TaskConfig) -> None:
        ...
        self.logger.info(f"📋 Registered background task: {config.name} "
    ...
    async def start_task(self, task_name: str) -> bool:
        ...
        if task_name not in self.tasks:
            self.logger.error(f"Cannot start unregistered task: {task_name}", 
                             extra={"subsys": "background_task"})
            return False
        ...
        if not task_state.config.enabled:
            self.logger.info(f"Skipping disabled task: {task_name}", 
                           extra={"subsys": "background_task", "task": task_name})
            return False
            
        if task_state.task and not task_state.task.done():
            self.logger.warning(f"Task {task_name} already running", 
                               extra={"subsys": "background_task", "task": task_name})
            return True
        ...
        try:
            ...
            task_state.task = asyncio.create_task(wrapped_func(), name=f"bg_{task_name}")
            ...
            self.logger.error(f"Failed to start task {task_name}: {e}", 
                             exc_info=True, extra={"subsys": "background_task", "task": task_name})
    ...
    def _wrap_task_function(self, task_state: TaskState) -> Callable[[], Awaitable[None]]:
        ...
        async def wrapped_task():
            ...
            try:
                ...
                self.logger.debug(f"🔄 Executing task function: {task_name}", 
                                 extra={"subsys": "background_task", "task": task_name})
                ...
                self.logger.info(f"✅ Task completed normally: {task_name}", 
                               extra={"subsys": "background_task", "task": task_name})
                ...
                self.logger.error(f"💥 Task failed: {task_name} - {error_details}", 
                                 exc_info=True, extra={"subsys": "background_task", "task": task_name})
    ...
    def record_heartbeat(self, task_name: str, message: Optional[str] = None) -> None:
        ...
        """
        if task_name not in self.tasks:
            self.logger.warning(f"Heartbeat from unknown task: {task_name}", 
                               extra={"subsys": "background_task"})
            return
        ...
        metrics.gauge(METRIC_BACKGROUND_LAST_HEARTBEAT, current_time, {"task": task_name})
        
        log_msg = f"💓 Heartbeat: {task_name}"
        if message:
            log_msg += f" - {message}"
            
        self.logger.debug(log_msg, extra={"subsys": "background_task", "task": task_name})
    ...
    async def start_watchdog(self) -> None:
        ...
        if self.watchdog_task and not self.watchdog_task.done():
            self.logger.warning("Watchdog already running", extra={"subsys": "background_task"})
            return
            
        self.running = True
        self.watchdog_task = asyncio.create_task(self._watchdog_loop(), name="bg_watchdog")
        
        self.logger.info(f"🐕 Started background task watchdog (interval: {self.watchdog_interval}s)", 
                        extra={"subsys": "background_task"})
    ...
    async def _check_single_task(self, task_name: str, task_state: TaskState, current_time: float) -> None:
        ...
        metrics.gauge(METRIC_BACKGROUND_STALENESS_SECONDS, staleness, {"task": task_name})
        
        # Check if task is stale
        if staleness > config.staleness_threshold and task_state.status == TaskStatus.HEALTHY:
            task_state.status = TaskStatus.STALE
            self.logger.warning(f"⏰ Task is stale: {task_name} (last heartbeat {staleness:.1f}s ago)", 
                               extra={"subsys": "background_task", "task": task_name})
        ...
        if task_state.task is None:
            ...
            if task_state.task.cancelled():
                if config.restart_policy in (RestartPolicy.ALWAYS, RestartPolicy.EXPONENTIAL_BACKOFF):
                    should_restart = True
        ...
        if should_restart and task_state.consecutive_failures >= config.max_consecutive_failures:
            should_restart = False
            if task_state.status != TaskStatus.FAILED:
                task_state.status = TaskStatus.FAILED
                self.logger.error(f"🚫 Task {task_name} exceeded max failures ({config.max_consecutive_failures}), giving up", 
                                 extra={"subsys": "background_task", "task": task_name})
    ...
    async def _restart_task(self, task_name: str, task_state: TaskState, reason: str) -> None:
        ...
        if delay > 0:
            self.logger.info(f"🔄 Restarting {task_name} in {delay:.1f}s: {reason}", 
                           extra={"subsys": "background_task", "task": task_name})
            await asyncio.sleep(delay)
        else:
            self.logger.info(f"🔄 Restarting {task_name}: {reason}", 
                           extra={"subsys": "background_task", "task": task_name})
    ...
    async def stop_all_tasks(self, timeout: float = 10.0) -> None:
        ...
        if tasks_to_cancel:
            self.logger.info(f"Waiting for {len(tasks_to_cancel)} tasks to stop...", 
                           extra={"subsys": "background_task"})
            ...
            try:
                ...
                self.logger.warning(f"Some tasks did not stop within {timeout}s timeout", 
                                  extra={"subsys": "background_task"})
