...
try:
    ...
    load_system_prompts = None  # type: ignore
...
class ObservabilityManager:
    ...
    async def initialize_observability_stack(self) -> bool:
        ...
        try:
            ...
            self.logger.info(
                "🔧 Starting observability stack initialization",
            ...
            self.logger.error(
                f"❌ Failed to initialize observability stack: {e}",
                exc_info=True,
                extra={"subsys": "observability"},
            )
            return False
    ...
    def _register_health_components(self) -> None:
        ...
        self.logger.info(
            f"📊 Registered {len(components)} components for health monitoring",
            extra={"subsys": "observability"},
        )
    ...
    def _configure_parallel_startup(self) -> None:
        ...
        self.logger.info(
            f"🚀 Configured parallel startup with {len(all_specs)} components",
            extra={"subsys": "observability"},
        )
    ...
    async def execute_startup_orchestration(self) -> Dict[str, Any]:
        ...
        if not parallel_startup:
            self.logger.info(
                "🔧 Parallel startup disabled, using sequential startup",
                extra={"subsys": "observability"},
            )
        ...
        try:
            ...
            self.logger.info(
                "🚀 Executing parallel startup orchestration",
                extra={"subsys": "observability"},
            )
            ...
            self.logger.error(
                f"💥 Parallel startup orchestration failed: {e}",
                exc_info=True,
                extra={"subsys": "observability"},
            )
    ...
    def _log_startup_summary(self, summary: Dict[str, Any], duration: float) -> None:
        ...
        try:
            ...
            status_icons = {"success": "✅", "degraded": "⚠️", "failed": "❌"}
    ...
    async def shutdown_observability_stack(self) -> None:
        ...
        try:
            ...
            self.logger.info(
                "✅ Observability stack shutdown complete",
                extra={"subsys": "observability"},
            )

        except Exception as e:
            self.logger.error(
                f"Error during observability shutdown: {e}",
                exc_info=True,
                extra={"subsys": "observability"},
            )
