    ...
    def _validate_constraint(self, constraint: ConfigConstraint) -> Optional[str]:
        ...
        if constraint.value_type:
            try:
                ...
                elif constraint.value_type == float:
                    typed_value = float(value) 
                elif constraint.value_type == bool:
        ...
        if constraint.min_value is not None or constraint.max_value is not None:
            try:
                numeric_value = float(typed_value) if constraint.value_type != int else int(typed_value)
...
class HealthMonitor:
    ...
    async def get_health_status(self) -> SystemHealth:
        ...
        try:
            ...
            self.logger.error(f"Failed to get health status: {e}", exc_info=True, extra={"subsys": "health"})
...
def validate_config_or_exit() -> None:
    ...
    try:
        ...
        logger.critical(f"Configuration validation system failed: {e}", exc_info=True, extra={"subsys": "config"})
        import sys
        sys.exit(2)
