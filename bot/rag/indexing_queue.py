...
class IndexingQueue:
    ...
    def __init__(
        ...
        logger.info(f"[RAG Indexing] Queue initialized: max_size={max_queue_size}, "
    ...
    async def enqueue_task(self, task: IndexingTask) -> bool:
        ...
        try:
            ...
            logger.debug(f"[RAG Indexing] ✔ Enqueued task: {task.source_id} "
            ...
            logger.warning(f"[RAG Indexing] ⚠ Queue overflow, dropping task: {task.source_id} "
    ...
    async def _process_task_sync(self, task: IndexingTask) -> bool:
        ...
        try:
            ...
            if success:
                ...
                logger.debug(f"[RAG Indexing] ✔ Synchronously processed: {task.source_id} "
    ...
    async def _process_task(self, task: IndexingTask, worker_id: int) -> None:
        ...
        try:
            logger.debug(f"[RAG Indexing] Worker {worker_id} processing: {task.source_id} "
            ...
            if success:
                ...
                logger.debug(f"[RAG Indexing] ✔ Worker {worker_id} completed: {task.source_id} "
                           f"({processing_time:.2f}s)")
            else:
    ...
    async def _handle_task_failure(self, task: IndexingTask, error_message: str, worker_id: int) -> None:
        ...
        if task.attempts < task.max_attempts:
            ...
            logger.warning(f"[RAG Indexing] Worker {worker_id} task failed, retrying in {backoff_delay}s: "
            ...
            logger.error(f"[RAG Indexing] ✖ Worker {worker_id} task failed permanently: "
