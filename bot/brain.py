    ...
    try:
        ...
        if "text" in response:
            ...
            if safe_text and safe_text.strip():
                logger.info(f"✅ Brain inference completed: {len(safe_text)} characters")
            ...
            try:
                ...
                fallback = f"I didn’t receive a usable response from the model just now. Could you rephrase or add a bit more detail about “{snippet}”?"
                logger.info(
                    "text.empty_fallback",
                    extra={
