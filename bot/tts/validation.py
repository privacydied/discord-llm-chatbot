...
def dump_environment_diagnostics() -> Dict[str, Any]:
    ...
    # Log the diagnostics
    logger.info(
        "Tokeniser discovery environment:",
        extra={"subsys": "tts", "event": "tokenizer.env.start"},
    )

    # Log PATH
    logger.info(
        f"PATH: {os.pathsep.join(path_dirs[:3])}{'...' if len(path_dirs) > 3 else ''}",
    ...
    logger.info(
        f"python.site: {site_pkg_display}{'...' if len(site_packages) > 5 else ''}",
    ...
    if diagnostics["espeak_binary"]:
        logger.info(
            f"espeak binary: FOUND ({diagnostics['espeak_binary']})",
    ...
    if diagnostics["espeak_ng_binary"]:
        logger.info(
            f"espeak-ng binary: FOUND ({diagnostics['espeak_ng_binary']})",
    ...
    logger.info(
        f"phonemizer module: {'FOUND' if diagnostics['phonemizer_module'] else 'NOT FOUND'}",
        extra={"subsys": "tts", "event": "tokenizer.env.module.phonemizer"},
    )

    logger.info(
        f"g2p_en module: {'FOUND' if diagnostics['g2p_en_module'] else 'NOT FOUND'}",
        extra={"subsys": "tts", "event": "tokenizer.env.module.g2p_en"},
    )

    logger.info(
        f"misaki module: {'FOUND' if diagnostics['misaki_module'] else 'NOT APPLICABLE'}",
...
def detect_available_tokenizers() -> Dict[str, bool]:
    ...
    if AVAILABLE_TOKENIZERS - {
        ...
        logger.warning(
            "No known tokenization methods found",
...
def select_tokenizer_for_language(
    ...
    if env_tokenizer:
        if env_tokenizer in available_set:
            logger.info(
                f"Using tokenizer from TTS_TOKENISER environment variable: {env_tokenizer}",
                extra={
                    "subsys": "tts",
                    "event": "tokenizer.env_override",
    ...
    for tokenizer in preferred_tokenizers:
        if tokenizer in available_set:
            logger.info(
                f"Selected tokenizer '{tokenizer}' for language '{language}'",
                extra={
                    "subsys": "tts",
                    "event": "tokenizer.selected",
                    "tokenizer": tokenizer,
                    "language": language,
                    "fallback": False,
                },
    ...
    if language.startswith("en") and not any(
        ...
        logger.error(
            f"No English phonetic tokenizer found. Required: {required}, Available: {available}",
            extra={
                "subsys": "tts",
                "event": "tokenizer.missing.critical",
                "language": language,
                "required": required,
                "available": available,
            },
    ...
    logger.warning(
        f"No preferred tokenizer available for language '{language}', using grapheme fallback",
        extra={"subsys": "tts", "event": "tokenizer.fallback", "language": language},
    )
...
def validate_voice_vector(voice_vector: np.ndarray, voice_id: str = None) -> bool:
    ...
    if len(voice_vector.shape) != 1 or voice_vector.shape[0] != expected_length:
        logger.error(
            f"Voice vector has unexpected shape: {voice_vector.shape}, expected (256,)",
            extra={"subsys": "tts", "event": "voice_validation", "voice_id": voice_id},
        )
        return False
    ...
    if vector_norm < 1e-6:
        logger.error(
            f"Voice vector has near-zero norm: {vector_norm}",
            extra={"subsys": "tts", "event": "voice_validation", "voice_id": voice_id},
        )
        return False
    ...
    if np.isnan(voice_vector).any() or np.isinf(voice_vector).any():
        logger.error(
            "Voice vector contains NaN or Inf values",
            extra={"subsys": "tts", "event": "voice_validation", "voice_id": voice_id},
        )
        return False
    ...
    if normalized_norm > 0.1:  # If norm is not close to 1.0
        logger.warning(
            f"Voice vector is not normalized (norm: {vector_norm:.4f})",
            extra={"subsys": "tts", "event": "voice_validation", "voice_id": voice_id},
        )
    ...
    if voice_id:
        ...
        if len(parts) >= 2:
            ...
            logger.debug(
                f"Voice ID {voice_id} has language code {lang_code}",
                extra={
                    "subsys": "tts",
                    "event": "voice_validation",
                    "voice_id": voice_id,
                },
            )

    logger.debug(
        f"Voice vector validated successfully (norm: {vector_norm:.4f}, length: {voice_vector.shape[0]})",
        extra={"subsys": "tts", "event": "voice_validation", "voice_id": voice_id},
    )
    return True
    """
    ...
    if voice_vector is None:
        logger.error(
            f"Voice vector is None for voice '{voice_id}'",
            extra={
                "subsys": "tts",
                "event": "voice.validate.none",
                "voice_id": voice_id,
            },
        )
    ...
    if voice_vector.shape != (256,):
        logger.error(
            f"Voice vector has incorrect shape: {voice_vector.shape}, expected (256,) for voice '{voice_id}'",
            extra={
                "subsys": "tts",
                "event": "voice.validate.shape",
                "voice_id": voice_id,
    ...
    if norm < 1e-6:
        logger.error(
            f"Voice vector has near-zero norm: {norm:.6f} for voice '{voice_id}'",
            extra={
                "subsys": "tts",
                "event": "voice.validate.norm",
                "voice_id": voice_id,
                "norm": norm,
            },
        )
...
def check_sample_rate_consistency(expected_rate: int, actual_rate: int) -> bool:
    ...
    if diff_percent < 1.0:
        logger.warning(
            f"Sample rates slightly different: expected {expected_rate}Hz, got {actual_rate}Hz",
    ...
    logger.error(
        f"Sample rate mismatch: expected {expected_rate}Hz, got {actual_rate}Hz",
        extra={"subsys": "tts", "event": "sample_rate"},
    )
    return False
...
def detect_gibberish_audio(audio_data: np.ndarray, sample_rate: int) -> bool:
    ...
    if mean_abs < 1e-4:
        logger.warning(
            f"Audio has very low amplitude (mean abs: {mean_abs}), likely gibberish",
    ...
    if zcr > 0.2:  # Threshold determined empirically
        logger.warning(
            f"Audio has high zero-crossing rate ({zcr:.4f}), likely gibberish",
            extra={"subsys": "tts", "event": "gibberish_detection"},
        )
        return True
    ...
    if np.max(np.abs(audio_data)) > 0.99:
        ...
        if clipping_ratio > 0.01:
            logger.warning(
                f"Audio has extensive clipping ({clipping_ratio:.4f}), likely distorted",
                extra={"subsys": "tts", "event": "gibberish_detection"},
            )
            return True
    ...
    if len(audio_mono) > segment_length * 2:  # Only check if audio is long enough
        for i in range(0, len(audio_mono) - segment_length, segment_length):
            ...
            if np.var(segment) < 1e-6 and np.mean(np.abs(segment)) > 1e-3:
                logger.warning(
                    f"Audio has constant segment at {i / sample_rate:.2f}s, likely stuck voice",
                    extra={"subsys": "tts", "event": "gibberish_detection"},
                )
                return True
...
def detect_gibberish_audio_with_metrics(
    ...
    zero_crossings = np.sum(np.abs(np.diff(np.signbit(audio_data).astype(int))))
    zcr = zero_crossings / (len(audio_data) - 1)
    metrics["zcr"] = float(zcr)

    # Log metrics for debugging
    logger.debug(
        f"Audio metrics: avg_abs={avg_abs:.6f}, rms={rms:.6f}, zcr={zcr:.6f}",
        extra={
            "subsys": "tts",
            "event": "gibberish.metrics",
            "avg_abs": avg_abs,
            "rms": rms,
            "zcr": zcr,
        },
    ...
    if avg_abs < 1e-4:
        is_gibberish = True
        logger.error(
            f"Audio output has very low volume: avg_abs={avg_abs:.6f}",
    ...
    if zcr > 0.4:  # This threshold may need tuning
        is_gibberish = True
        logger.error(
            f"Audio output has abnormally high zero-crossing rate: zcr={zcr:.6f}",
