...
class StructuredJsonFormatter(logging.Formatter):
    ...
    FROZEN_KEYS = {
        'ts', 'level', 'name', 'subsys', 'guild_id', 
        'user_id', 'msg_id', 'event', 'detail'
    }
...
class LoggingEnforcer:
    ...
    def enforce_startup_assertion(self) -> None:
        ...
        pretty_active = any(
            isinstance(h, (RichHandler, PrettyConsoleHandler)) 
        ...
        if not pretty_active:
            print("❌ FATAL: Pretty console handler missing from logging configuration", file=sys.stderr)
            sys.exit(1)
        
        if not jsonl_active:
            print("❌ FATAL: JSONL file handler missing from logging configuration", file=sys.stderr)  
            sys.exit(1)
...
if __name__ == "__main__":
    ...
    logger.info_event("router.message_processed", {
        "latency_ms": 245.7,
        "items_count": 3,
        "streaming": True
    })
    logger.warning("Performance degradation detected")
    logger.warning_event("router.budget_exceeded", {
        "family": "tweet_syndication", 
        "elapsed_ms": 1250,
        "budget_ms": 1000
    })
    logger.error("Critical system failure")
    logger.error_event("router.hard_deadline_exceeded", {
        "family": "stt_processing",
        "elapsed_ms": 5000,
        "deadline_ms": 3000
    })
