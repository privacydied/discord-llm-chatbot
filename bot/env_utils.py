...
def resolve_env(
    ...
    if new_value is not None and legacy_value is not None and new_value != legacy_value:
        logger.info(
            f"Using {name_new}='{new_value}', ignoring legacy {name_legacy}='{legacy_value}'",
            extra={
                "subsys": "config",
                "event": "env.resolve.both_set",
                "new_var": name_new,
                "legacy_var": name_legacy,
            },
        )
        return new_value
    ...
    if legacy_value is not None:
        logger.warning(
            f"Legacy environment variable {name_legacy} is deprecated. Please use {name_new} instead.",
            extra={
                "subsys": "config",
                "event": "env.resolve.legacy_only",
                "legacy_var": name_legacy,
                "new_var": name_new,
            },
        )
