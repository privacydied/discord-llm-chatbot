...
class MediaIngestionManager:
    ...
    async def _extract_media_with_retry(self, url: str) -> Tuple[bool, Optional[Dict], Optional[str]]:
        """
                ...
                try:
                    ...
                    if media_result.metadata:
                        ...
                        for key, value in media_result.metadata.items():
                            if key in ['title', 'uploader', 'duration_seconds', 'source']:
        ...
        try:
            ...
            if fallback_result.content and "Screenshot available at:" in fallback_result.content:
                ...
                if not vision_response or vision_response.error:
                    return BotAction(content="I couldn't understand the content of the URL.", error=True)
    ...
    def _log_success_metrics(self, url: str, message, media_result: MediaIngestionResult, probe_result: ProbeResult):
    ...
    def _log_fallback_metrics(self, url: str, message, fallback_result: MediaIngestionResult, probe_result: ProbeResult):
        ...
        if fallback_result.processing_time_ms:
            self._metric_observe('media_ingestion_duration_ms', fallback_result.processing_time_ms, labels)
    ...
    def _metric_observe(self, metric_name: str, value: float, labels: Optional[Dict[str, str]] = None):
