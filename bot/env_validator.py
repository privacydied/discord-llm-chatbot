...
def validate_environment() -> Tuple[bool, Optional[str]]:
    ...
    if missing_core:
        return (
            False,
            f"Missing required environment variables: {', '.join(missing_core)}",
        )
    ...
    if backend == "openai":
        ...
        if missing_ollama:
            return (
                False,
                f"Ollama backend selected but missing required variables: {', '.join(missing_ollama)}",
            )
    else:
        return (
            False,
            f"Invalid TEXT_BACKEND value: {backend}. Must be 'openai' or 'ollama'.",
        )
...
def get_environment_info() -> Dict[str, Any]:
    ...
    info = {
        ...
        "model": os.environ.get(
            "OPENAI_TEXT_MODEL",
            os.environ.get("OPENAI_MODEL", os.environ.get("OLLAMA_MODEL", "unknown")),
        ),
