...
class OptimizedRouter:
    ...
    def __init__(self, bot: "DiscordBot"):
        ...
        self.fast_classify_enabled = self.config.get(
        ...
        self.logger.info(
            f"🔍 Vision initialization debug: VISION_ENABLED={VISION_ENABLED}, config_enabled={self.config.get('VISION_ENABLED', 'NOT_SET')}"
        )
        ...
        if VISION_ENABLED and self.config.get("VISION_ENABLED", False):
            ...
            try:
                ...
                self.logger.error(
                    f"❌ Failed to initialize Vision system: {e}", exc_info=True
                )
                self._vision_intent_router = None
                self._vision_orchestrator = None
        else:
            self.logger.warning(
                f"⚠️ Vision system NOT initialized - VISION_ENABLED={VISION_ENABLED}, config={self.config.get('VISION_ENABLED', 'NOT_SET')}"
            )
    ...
    def _is_reply_to_bot(self, message: Message) -> bool:
        ...
        return (
            hasattr(resolved_message, "author")
            and resolved_message.author == self.bot.user
        )
    ...
    async def dispatch_message(self, message: Message) -> Optional[BotAction]:
        ...
        try:
            ...
            self.metrics.plan_duration_ms = (
                self.metrics.plan_duration_ms * self.metrics.processed_messages
                + plan_duration_ms
            ...
            execution = OptimizedExecution(
                message_id=message.id, plan_result=plan_result, started_at=start_time
            )
                ...
                self.logger.info(
                    ...
                    extra={
                        ...
                        "detail": {
                            ...
                            "plan_duration_ms": plan_duration_ms,
                            "execution_duration_ms": execution.elapsed_ms
                            - plan_duration_ms,
    ...
    async def _execute_optimized_plan(
        self, message: Message, execution: OptimizedExecution
    ) -> BotAction:
    ...
    async def _handle_text_only(self, message: Message, text_content: str) -> BotAction:
        ...
        return await self.concurrency_manager.run_light_task(
            ...
            timeout=self.budget_manager.budget_configs[
                BudgetFamily.TEXT_INFERENCE
            ].hard_deadline_ms
            / 1000,
        )
    ...
    async def _handle_without_streaming(
        ...
        for item, classification in plan.items:
            try:
                result = await self._process_item_optimized(
                    item, classification, message, execution
                )
                if result:
        ...
        if results:
            ...
            return BotAction(
                content="I couldn't process any of the items in your message."
            )
    ...
    async def _process_item_optimized(
        ...
        if classification.is_twitter:
        ...
        elif classification.modality in [
            ...
            return await self._handle_general_url_optimized(
                item, classification, execution
            )
        else:
    ...
    async def _handle_twitter_optimized(
        ...
        async def tweet_flow():
            ...
            cached_result, hit = await self.single_flight_cache.get_or_compute(
                ...
                cache_key,
                lambda: self._fetch_tweet_with_fallbacks(
                    url, classification.extracted_id
                ),
            )
        ...
        return await self.concurrency_manager.run_network_task(
            tweet_flow(), task_name="twitter_flow"
        )

    async def _fetch_tweet_with_fallbacks(
        self, url: str, tweet_id: Optional[str]
    ) -> str:
        ...
        try:
            async with self.budget_manager.execute_with_budget(
                BudgetFamily.TWEET_SYNDICATION, f"syndication_{tweet_id or 'unknown'}"
            ):
            ...
            self.logger.warning(
                f"⚠️ Tweet syndication budget exceeded ({e.elapsed_ms}ms), switching to web extraction"
            )

            # Fallback to web extraction with different budget
            try:
                async with self.budget_manager.execute_with_budget(
                    BudgetFamily.WEB_EXTRACTION, f"web_extract_{tweet_id or 'unknown'}"
                ):
                    result = await self._fetch_tweet_web(url)
                    self.logger.info(
                        f"✔️ Tweet web extraction succeeded for {url[:40]}..."
                    )
                    return result
            ...
            self.metrics.hard_failures += 1
            self.logger.error(
                f"❌ Tweet syndication hard deadline exceeded: {e.elapsed_ms}ms"
            )
            return (
                f"❌ Tweet processing cancelled (deadline exceeded: {e.elapsed_ms}ms)"
            )

        except Exception as e:
            self.logger.warning(
                f"⚠️ Tweet syndication failed: {e}, trying web extraction"
            )
            ...
            try:
                async with self.budget_manager.execute_with_budget(
                    ...
                    result = await self._fetch_tweet_web(url)
                    self.logger.info("✔️ Tweet web extraction fallback succeeded")
                    return result

            except Exception as fallback_error:
                self.metrics.hard_failures += 1
                self.logger.error(
                    f"❌ All tweet extraction methods failed: {fallback_error}"
                )
    ...
    async def _fetch_tweet_syndication(self, url: str, tweet_id: Optional[str]) -> str:
        ...
        async def fetch_syndication():
            async with self.budget_manager.execute_with_budget(
                BudgetFamily.TWEET_SYNDICATION, f"syndication_{tweet_id}"
            ):
        ...
        result, cache_hit = await self.cache.get_or_compute(
            CacheFamily.TWEET_TEXT, cache_key, fetch_syndication
        )
    ...
    async def _fetch_tweet_web(self, url: str) -> str:
        ...
        async def web_extraction():
            async with self.budget_manager.execute_with_budget(
                BudgetFamily.TWEET_WEB_TIER_A, f"web_tier_a_{hash(url)}"
            ):
