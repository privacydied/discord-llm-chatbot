...
class MediaIngestionConfig:
    ...
    probe_cache_ttl_seconds: int = int(
        os.getenv("MEDIA_PROBE_CACHE_TTL", "300")
    )  # 5 minutes
    probe_timeout_seconds: int = int(
        os.getenv("MEDIA_PROBE_TIMEOUT", "10")
    )  # 10 seconds
    probe_cache_dir: Path = Path(
        os.getenv("MEDIA_PROBE_CACHE_DIR", "cache/media_probes")
    )
    ...
    # Feature flags
    enable_media_ingestion: bool = (
        os.getenv("ENABLE_MEDIA_INGESTION", "true").lower() == "true"
    )
    enable_twitter_video_detection: bool = (
        os.getenv("ENABLE_TWITTER_VIDEO_DETECTION", "true").lower() == "true"
    )
    enable_contextual_brain: bool = (
