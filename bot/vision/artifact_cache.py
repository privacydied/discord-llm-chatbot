...
class VisionArtifactCache:
    ...
    async def store_artifact(self, content: bytes, filename: str, optimize_for_discord: bool = True) -> str:
        ...
        if len(content) > self.max_file_size_mb * 1024 * 1024:
            msg = f"File too large: {len(content) / 1024 / 1024:.1f}MB > {self.max_file_size_mb}MB"
        ...
        async with self._get_lock(content_hash):
            ...
            try:
                ...
                if not is_valid:
                    msg = f"Unsupported or invalid file format: {filename}"
                    raise ValueError(msg)
                ...
                self.logger.exception(f"Failed to store artifact: {e}")
    ...
    async def retrieve_artifact(self, content_hash: str) -> tuple[bytes, ArtifactMetadata] | None:
        ...
        async with self._get_lock(content_hash):
            ...
            try:
                ...
                self.logger.exception(f"Failed to retrieve artifact {content_hash[:8]}: {e}")
                return None
    ...
    async def delete_artifact(self, content_hash: str) -> bool:
        ...
        async with self._get_lock(content_hash):
            ...
            try:
                ...
                self.logger.exception(f"Failed to delete artifact {content_hash[:8]}: {e}")
                return False

    async def get_discord_file_info(self, content_hash: str) -> dict[str, Any] | None:
    ...
    def _validate_file_format(self, content: bytes, filename: str) -> tuple[FileFormat, bool]:
        ...
        if content.startswith((b"\x00\x00\x00\x18ftypmp4", b"\x00\x00\x00 ftypmp41")):
    ...
    async def _extract_dimensions(self, file_path: Path, file_format: FileFormat) -> tuple[int, int] | None:
    ...
    async def _extract_duration(self, file_path: Path, file_format: FileFormat) -> float | None:
    ...
    async def _background_cleanup(self) -> None:
        ...
        while True:
            try:
                ...
                # Log stats
                self.logger.debug(
                    f"Cache cleanup completed - total_files: {self._stats.total_files}, total_size_mb: {round(self._stats.total_size_bytes / 1024 / 1024, 1)}, hit_rate: {self._stats.hit_count / max(self._stats.hit_count + self._stats.miss_count, 1):.2f}, expired_cleaned: {expired_count}",
                )
