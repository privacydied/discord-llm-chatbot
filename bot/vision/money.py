...
class Money:
    ...
    def __init__(self, value: str | float | Decimal | Money) -> None:
    ...
    @classmethod
    def from_cents(cls, cents: float) -> Money:
        """Create Money from cent value (useful for provider APIs that return cents)."""
        return cls(Decimal(str(cents)) / 100)

    @classmethod
    def from_credits(cls, credits: float, credits_per_dollar: float = 100) -> Money:
    ...
    def __add__(self, other: Money | Decimal | float) -> Money:
        if not isinstance(other, Money):
            other = Money(other)
        return Money(self._amount + other._amount)

    def __sub__(self, other: Money | Decimal | float) -> Money:
        if not isinstance(other, Money):
            other = Money(other)
        return Money(self._amount - other._amount)
    ...
    def __lt__(self, other: Money | Decimal | float) -> bool:
        if not isinstance(other, Money):
            other = Money(other)
        return self._amount < other._amount

    def __le__(self, other: Money | Decimal | float) -> bool:
        if not isinstance(other, Money):
            other = Money(other)
        return self._amount <= other._amount

    def __gt__(self, other: Money | Decimal | float) -> bool:
        if not isinstance(other, Money):
            other = Money(other)
        return self._amount > other._amount

    def __ge__(self, other: Money | Decimal | float) -> bool:
        if not isinstance(other, Money):
            other = Money(other)
        return self._amount >= other._amount
    ...
    # Utility methods
    def clamp_minimum(self, minimum: Money | Decimal | float = 0) -> Money:
    ...
    def ratio_to(self, other: Money | Decimal | float) -> Decimal:
