...
class VisionBudgetManager:
    ...
    def _reset_expired_periods(self, budget: UserBudget) -> None:
        ...
        if budget.daily_reset_time:
            ...
            if now >= reset_time:
                ...
                budget.daily_reset_time = (now + timedelta(days=1)).replace(
        ...
        if budget.weekly_reset_time:
            ...
            if now >= reset_time:
                ...
                budget.weekly_reset_time = (now + timedelta(days=days_until_monday)).replace(
                    hour=0, minute=0, second=0, microsecond=0
                ).isoformat()
        else:
    ...
    async def reserve_budget(self, user_id: str, amount: Money) -> None:
        ...
        async with self._get_user_lock(user_id):
            ...
            # Log transaction
            self._append_jsonl(self.transactions_file, TransactionRecord(
