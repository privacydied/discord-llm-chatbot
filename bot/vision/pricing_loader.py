...
class PricingTable:
    ...
    def estimate_cost(
        ...
        # Calculate base cost
        if task in [VisionTask.TEXT_TO_VIDEO, VisionTask.IMAGE_TO_VIDEO, VisionTask.VIDEO_GENERATION]:
    ...
    def get_max_discrepancy_ratio(self) -> Decimal:
        ...
        return Decimal(str(
            self.pricing_data.get("estimation_config", {}).get("max_discrepancy_ratio", 5.0)
        ))
    
    def get_warning_discrepancy_ratio(self) -> Decimal:
        """Get discrepancy ratio threshold for warnings"""
        return Decimal(str(
            self.pricing_data.get("estimation_config", {}).get("warning_discrepancy_ratio", 2.0)
        ))
