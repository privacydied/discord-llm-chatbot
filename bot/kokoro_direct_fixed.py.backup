            ...
            try:
                ...
                if "'EspeakWrapper' object has no attribute 'set_data_path'" in str(e):
                    ...
                    try:
                        ...
                        logger.error(f"Monkey patch failed: {patch_e} [subsys: tts, event: load_model.patch_failed]")
            except Exception as e:
                logger.error(f"Failed to load ONNX model: {e} [subsys: tts, event: init.onnx_error]", exc_info=True)
                raise
            except Exception as e:
                logger.error(f"Failed to load kokoro_onnx or onnxruntime: {e} [subsys: tts, event: load_model.error.import]", exc_info=True)
                raise

        # Load voices
        try:
            self.voices = np.load(self.voices_path)
            logger.debug(f"Loaded {len(self.voices)} voices [subsys: tts, event: init.voices_loaded]")
        except Exception as e:
            logger.error(f"Failed to load voices: {e} [subsys: tts, event: init.voices_error]", exc_info=True)
            raise
...
# ...
    def _create_audio(self, phonemes: str, voice_embedding: np.ndarray, speed: float = 1.0, tokens: Optional[List[int]] = None) -> Tuple[np.ndarray, int]:
        ...
        try:
            ...
            # Load voice embedding with deterministic default
            voice_embedding = self._get_voice_embedding(voice, voice_id_or_embedding)
            if voice_embedding is None:
                logger.warning("No voice embedding found; using zeros [subsys: tts, event: create.warning.no_voice]")
            ...
            if 'style' in input_names:
                ...
                inputs[input_names[1]] = voice_embedding.reshape(1, -1).astype(np.float32)
            ...
            logger.debug(f"ONNX inputs prepared: {[name for name in inputs.keys()]} [subsys: tts, event: create_audio.inputs_prepared]")
