    ...
    try:
        ...
        for module_name, cog_name in module_imports.items():
            try:
                ...
                logging.error(
                    f"[Commands Setup] ❌ Failed to import {module_name}: {import_error}",
                    exc_info=True,
                )
                continue
        ...
        for cog_name, module in modules.items():
            try:
                ...
                logging.error(
                    f"[Commands Setup] ❌ Failed to load {cog_name}: {cog_error}",
                    exc_info=True,
                )
                failed_loads += 1
                continue
        ...
        logging.error(
            f"[Commands Setup] ❌ Critical error during command setup: {e}",
            exc_info=True,
        )
        raise
