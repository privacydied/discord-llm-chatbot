...
class ImageUpgradeManager:
    ...
    async def store_upgrade_context(self, message_id: int, url: str, syn_data: Dict[str, Any], 
                                  source: str, original_analysis: List[str]) -> None:
        """
    ...
    async def _generate_detailed_caption(self, photos: List[Dict[str, Any]], 
                                       original_analysis: List[str], url: str) -> str:
        """Generate detailed image captions with composition and attributes. [CA]"""
        try:
            detailed_parts = ["**🖼️ Detailed Description**"]
            
            for idx, (photo, analysis) in enumerate(zip(photos, original_analysis), start=1):
                ...
                detailed_prompt = (
                    f"Provide a detailed description of this image including composition, "
                    f"colors, lighting, mood, objects, people, text, and any notable details. "
                    f"Be thorough but objective."
                )
                    ...
                    if router and hasattr(router, '_vl_describe_image_from_url'):
                        ...
                        if detailed_analysis:
                            if len(photos) == 1:
                                ...
                                detailed_parts.append(f"**Image {idx}/{len(photos)}:**\n{detailed_analysis}")
                        else:
                            detailed_parts.append(f"**Image {idx}/{len(photos)}:** Analysis unavailable")
                    else:
                        # Fallback to original analysis
                        detailed_parts.append(f"**Image {idx}/{len(photos)}:** {analysis}")
                        
                except Exception as detail_err:
                    logger.error(f"❌ Detailed analysis failed for image {idx}: {detail_err}")
                    detailed_parts.append(f"**Image {idx}/{len(photos)}:** Could not generate detailed description")
    ...
    async def _generate_ocr_details(self, photos: List[Dict[str, Any]], url: str) -> str:
        ...
        try:
            ...
            for idx, photo in enumerate(photos, start=1):
                ...
                # OCR-focused prompt
                ocr_prompt = (
                    f"Extract and transcribe ALL visible text from this image. Include text on signs, "
                    f"labels, documents, screens, clothing, or any other objects. Preserve formatting "
                    f"where possible and indicate if text is partially obscured or unclear."
                )
                ...
                try:
                    ...
                    if router and hasattr(router, '_vl_describe_image_from_url'):
                        ...
                        if ocr_result and any(keyword in ocr_result.lower() for keyword in ["text", "says", "reads", '"']):
                            found_text = True
                        ...
                        ocr_parts.append(f"**Image {idx}/{len(photos)}:** OCR analysis unavailable")
    ...
    async def _generate_tags(self, original_analysis: List[str], url: str) -> str:
        ...
        try:
            ...
            common_objects = ["person", "people", "man", "woman", "child", "car", "building", "tree", "food", 
                            "animal", "dog", "cat", "book", "phone", "computer", "sign", "text", "outdoor", 
                            "indoor", "street", "room", "kitchen", "office"]
    ...
    async def _generate_explanation(self, syn_data: Dict[str, Any], 
                                  original_analysis: List[str], url: str) -> str:
