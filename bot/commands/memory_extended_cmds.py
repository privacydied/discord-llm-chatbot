...
class ExtendedMemoryCommands(commands.Cog):
    ...
    async def memory_status(self, ctx):
        ...
        try:
            ...
            if enabled and service:
                queue_size = len(service.queue._queue) if hasattr(service.queue, '_queue') else 0
    ...
    async def memory_disable(self, ctx):
        ...
        try:
            ...
            if ctx.guild:
                is_guild_admin = getattr(ctx.author, 'guild_permissions', None) and ctx.author.guild_permissions.administrator
    ...
    async def memory_disable(self, ctx):
        ...
        try:
            ...
            if save_profile(profile, caller_id=str(ctx.author.id)):
                await ctx.send("✅ Memory ingestion has been disabled for your profile.")
            else:
