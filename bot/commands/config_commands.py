...
from ..config_reload import manual_reload_command, get_config_for_debug, get_config_version
...
class ConfigCommands(commands.Cog):
    ...
    async def reload_config(self, ctx):
        ...
        try:
            ...
            await ctx.reply("❌ You need administrator permissions to reload configuration.", mention_author=False)
        except Exception as e:
            logger.error(f"❌ Manual config reload failed: {e}")
            await ctx.reply("❌ Failed to reload configuration. Check logs for details.", mention_author=False)
    ...
    async def config_status(self, ctx):
        ...
        try:
            ...
            key_settings = []
            important_keys = [
                'TEXT_BACKEND', 'OPENAI_TEXT_MODEL', 'OLLAMA_MODEL', 
                'TTS_BACKEND', 'STT_ENGINE', 'LOG_LEVEL',
                'MAX_USER_MEMORY', 'MAX_SERVER_MEMORY', 'HISTORY_WINDOW'
            ]
            ...
            await ctx.reply("❌ You need administrator permissions to view configuration status.", mention_author=False)
        except Exception as e:
