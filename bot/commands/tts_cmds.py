...
class TTSCommands(commands.Cog):
    ...
    async def speak(
        ...
        if text:
            ...
            logging.debug(
                f"🔊 Delegating !speak with text to !say handler: '{text[:30]}...'",
                extra={
                    "subsys": "tts_cmds",
                    "event": "speak.with_text",
                    "user_id": user_id,
            ...
            logging.debug(
                f"🔊 Setting one-time TTS flag for user {user_id}",
                extra={
                    "subsys": "tts_cmds",
                    "event": "speak.one_time_flag",
                    "user_id": user_id,
                    "channel_id": channel_id,
                },
            )
