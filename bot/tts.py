...
class TTSManager:
    ...
    async def load_model(self):
        ...
        try:
            # Provider probe with timeout
            import onnxruntime as ort
            from concurrent.futures import ThreadPoolExecutor
            
            with ThreadPoolExecutor() as executor:
                future = executor.submit(ort.get_available_providers)
                providers = await asyncio.wait_for(
                    asyncio.get_event_loop().run_in_executor(None, future),
                    timeout=5.0
                )
            ...
            logging.error(f"ONNX initialization failed: {e}")
            self.set_available(False)
