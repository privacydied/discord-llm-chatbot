...
class KokoroDirect:
    ...
    def _select_phonemiser(self, language: str) -> str:
        ...
        try:
            ...
            try:
                if getattr(reg, "_available_tokenizers", None) and not getattr(
    ...
    def _load_model(self) -> None:
        ...
        try:
            ...
            try:
                ...
                if "'EspeakWrapper' object has no attribute 'set_data_path'" in str(e):
                    logger.error(
                        f"kokoro-onnx initialization error: {e} [subsys: tts, event: load_model.error]"
                    )

                    # Try to monkey patch at runtime as a last resort
                    try:
                        # Monkey patch code would go here
                        pass
                    except Exception as patch_e:
                        logger.error(
                            f"Monkey patch failed: {patch_e} [subsys: tts, event: load_model.patch_failed]"
                        )
            except Exception as e:
                logger.error(
                    f"Failed to load ONNX model: {e} [subsys: tts, event: init.onnx_error]",
                    exc_info=True,
                )
                raise
        except Exception as e:
            logger.error(
                f"Failed to load kokoro_onnx or onnxruntime: {e} [subsys: tts, event: load_model.error.import]",
                exc_info=True,
            )
            raise
        ...
        try:
            ...
            logger.debug(
                f"Loaded {len(self.voices)} voices [subsys: tts, event: init.voices_loaded]"
            )
        except Exception as e:
            logger.error(
                f"Failed to load voices: {e} [subsys: tts, event: init.voices_error]",
                exc_info=True,
            )
            raise
    ...
    def _detect_tokenization_methods(self, log_discovery: bool = False) -> None:
        ...
        if hasattr(self.tokenizer, "encode") and callable(
            getattr(self.tokenizer, "encode")
        ):
            self.available_tokenization_methods.add(TokenizationMethod.PHONEME_ENCODE)
            if log_discovery:
                logger.debug(
                    "Found tokenizer.encode method [subsys: tts, event: tokenizer.method.encode]"
                )

        if hasattr(self.tokenizer, "phoneme_to_id") and callable(
            getattr(self.tokenizer, "phoneme_to_id")
        ):
            self.available_tokenization_methods.add(TokenizationMethod.PHONEME_TO_ID)
            if log_discovery:
                logger.debug(
                    "Found tokenizer.phoneme_to_id method [subsys: tts, event: tokenizer.method.phoneme_to_id]"
                )

        if hasattr(self.tokenizer, "text_to_ids") and callable(
            getattr(self.tokenizer, "text_to_ids")
        ):
            self.available_tokenization_methods.add(TokenizationMethod.TEXT_TO_IDS)
            if log_discovery:
                logger.debug(
                    "Found tokenizer.text_to_ids method [subsys: tts, event: tokenizer.method.text_to_ids]"
                )

        if hasattr(self.tokenizer, "g2p_pipeline") and callable(
            getattr(self.tokenizer, "g2p_pipeline")
        ):
            self.available_tokenization_methods.add(TokenizationMethod.G2P_PIPELINE)
            if log_discovery:
                logger.debug(
                    "Found tokenizer.g2p_pipeline method [subsys: tts, event: tokenizer.method.g2p_pipeline]"
                )
        ...
        if _HAVE_ESPEAK:
            ...
            if log_discovery:
                logger.debug(
                    f"Found espeak at {_HAVE_ESPEAK} [subsys: tts, event: tokenizer.external.espeak]"
                )
        ...
        if _HAVE_PHONEMIZER:
            ...
            if log_discovery:
                logger.debug(
                    "Found phonemizer package [subsys: tts, event: tokenizer.external.phonemizer]"
                )
        ...
        if _HAVE_MISAKI:
            ...
            if log_discovery:
                logger.debug(
                    "Found misaki package [subsys: tts, event: tokenizer.external.misaki]"
                )
        ...
        if TokenizationMethod.PHONEME_ENCODE in self.available_tokenization_methods:
            ...
            if log_discovery:
                logger.warning(
                    "No known tokenization methods found [subsys: tts, event: tokenizer.method.none]"
                )
    ...
    def create(
        ...
        **_: Any,
        ...
        if phonemes:
            ...
            try:
                # Direct IPA synthesis using real model vocabulary
                from bot.tts.eng_g2p_local import _ipa_to_ids

                token_ids = _ipa_to_ids(phonemes)
                gen = self._generate_audio_from_ids(
                    phonemes, voice_embedding, speed, token_ids
                )
        ...
        if os.getenv("TTS_TOKENISER") and self._use_tokenizer:
            ...
            if (
                os.getenv("TTS_TOKENISER", "").strip().lower() == "espeak"
                and not espeak_available
            ):
                log.warning(
                    "Requested TTS_TOKENISER='espeak', but espeak is not available"
                )
    ...
    def _validate_language_resources(self) -> None:
        ...
        if self.tokenizer is None:
            ...
            if self._use_tokenizer:
                logger.error(
                    "No tokenizer available [subsys: tts, event: language.error.no_tokenizer]"
                )
            return
        ...
        if self.tokenization_method != TokenizationMethod.UNKNOWN:
            logger.info(
                f"Using tokenization method: {self.tokenization_method.value} [subsys: tts, event: language.tokenization_method]"
            )
        else:
            logger.debug(
                f"No tokenization method available for language '{self.language}' [subsys: tts, event: language.tokenization_method.none]"
            )

    def _synthesize_from_ipa(
        self, phonemes: str, voice: str, lang: str, speed: float
    ) -> Path:
        """
        ...
        try:
            ...
            if token_ids:
                audio, sr = self._create_audio(
                    phonemes, self.voices_data.get(voice, None), speed, tokens=token_ids
                )
    ...
    def tokenize_text(self, text: str) -> Tuple[List[int], TokenizationMethod]:
        ...
        if (
            lex_changed
            and TokenizationMethod.PHONEME_TO_ID in self.available_tokenization_methods
        ):
            try:
                token_ids: List[int] = []
                ...
                for piece in pieces:
                    try:
                        ...
                        token_ids.extend(
                            [self.tokenizer.phoneme_to_id(ch) for ch in piece]
                        )
                if token_ids and all(
                    isinstance(i, (int, np.integer)) for i in token_ids
                ):
        ...
        if TokenizationMethod.PHONEME_ENCODE in self.available_tokenization_methods:
            try:
                ...
                logger.warning(
                    f"Failed to tokenize with phoneme_encode: {e} [subsys: tts, event: tokenize.error.phoneme_encode]"
                )
        ...
        if (
            ...
            try:
                ...
                errors.append(f"phoneme_to_id: {e}")
                logger.warning(
                    f"Failed to tokenize with phoneme_to_id: {e} [subsys: tts, event: tokenize.error.phoneme_to_id]"
                )
        ...
        if TokenizationMethod.TEXT_TO_IDS in self.available_tokenization_methods:
            try:
                ...
                logger.warning(
                    f"Failed to tokenize with text_to_ids: {e} [subsys: tts, event: tokenize.error.text_to_ids]"
                )
        ...
        if TokenizationMethod.G2P_PIPELINE in self.available_tokenization_methods:
            try:
                ...
                logger.warning(
                    f"Failed to tokenize with g2p_pipeline: {e} [subsys: tts, event: tokenize.error.g2p_pipeline]"
                )

        # Fallback: ASCII grapheme fallback (treat each character as a token)
        try:
            logger.debug(
                "No other tokenization methods worked, using grapheme fallback [subsys: tts, event: tokenize.fallback.grapheme]"
            )
            ...
            logger.error(
                f"Even grapheme fallback tokenization failed: {e} [subsys: tts, event: tokenize.error.grapheme_fallback]"
            )
    ...
    def _load_voices(self) -> None:
        ...
        try:
            ...
            if not voices_path.exists():
                logger.error(
                    f"Voices file not found: {voices_path} [subsys: tts, event: load_voices.error, path: {voices_path}]"
                )
                return

            # Load NPZ file
            logger.debug(
                f"Loading NPZ voices from: {voices_path} [subsys: tts, event: load_voices.npz, path: {voices_path}]"
            )
            npz_data = np.load(voices_path, allow_pickle=True)

            if hasattr(npz_data, "files"):
                logger.debug(
                    f"NPZ file contains {len(npz_data.files)} voices [subsys: tts, event: load_voices.npz.count, voice_count: {len(npz_data.files)}]"
                )
                ...
                logger.info(
                    f"Loaded {len(self.voices)} voices from NPZ file [subsys: tts, event: load_voices.success, voice_count: {len(self.voices)}]"
                )
            else:
                logger.error(
                    "Invalid NPZ file format (no 'files' attribute) [subsys: tts, event: load_voices.error.format, path: {voices_path}]"
                )
        except Exception as e:
            logger.error(
                f"Failed to load voices: {e} [subsys: tts, event: load_voices.error, error: {e}]",
                exc_info=True,
            )
    ...
    def _create_audio(
        ...
        try:
            if tokens is not None:
                ...
                logger.debug(
                    f"Using pre-tokenized tokens: {len(token_ids)} tokens [subsys: tts, event: create_audio.pre_tokenized]"
                )
                ...
                method_used = TokenizationMethod.GRAPHEME_FALLBACK
                logger.debug(
                    f"Using grapheme tokens (tokenizer disabled): {len(token_ids)} tokens [subsys: tts, event: create_audio.grapheme_disabled]"
                )
            else:
                token_ids, method_used = self.tokenize_text(phonemes)
                logger.debug(
                    f"Tokenized {len(phonemes)} chars to {len(token_ids)} tokens using {method_used.value} [subsys: tts, event: create_audio.tokenize, method: {method_used.value}]"
                )

            # Verify we have non-empty token sequence
            if not token_ids or len(token_ids) == 0:
                logger.error(
                    "Tokenization produced empty token sequence [subsys: tts, event: create_audio.error.empty_tokens]"
                )
                raise ValueError("Tokenization produced empty token sequence")

        except Exception as e:
            logger.error(
                f"Failed to tokenize text: {e} [subsys: tts, event: create_audio.error.tokenize]",
                exc_info=True,
            )
        ...
        try:
            ...
            logger.debug(
                f"Model input names: {input_names} [subsys: tts, event: create_audio.input_names]"
            )
            ...
            if "input_ids" in input_names:
                inputs["input_ids"] = np.array([token_ids], dtype=np.int32)
            elif "tokens" in input_names:
                inputs["tokens"] = np.array([token_ids], dtype=np.int32)
            elif "text" in input_names:
                inputs["text"] = np.array([token_ids], dtype=np.int32)
            else:
                # Use first input as token input
                inputs[input_names[0]] = np.array([token_ids], dtype=np.int32)

            # voice_embedding is provided by caller; ensure fallback to zeros if missing
            if voice_embedding is None:
                logger.warning(
                    "No voice embedding found; using zeros [subsys: tts, event: create.warning.no_voice]"
                )
                voice_embedding = np.zeros((1, 256), dtype=np.float32)

            if "style" in input_names:
                inputs["style"] = voice_embedding.reshape(1, -1).astype(np.float32)
            elif "speaker" in input_names:
                inputs["speaker"] = voice_embedding.reshape(1, -1).astype(np.float32)
            elif len(input_names) > 1:
                # Use second input for voice embedding
                inputs[input_names[1]] = voice_embedding.reshape(1, -1).astype(
                    np.float32
                )
            ...
            if speed != 1.0:
                ...
                elif "length_scale" in input_names:
                    inputs["length_scale"] = np.array([1.0 / speed], dtype=np.float32)

            logger.debug(
                f"ONNX inputs prepared: {[name for name in inputs.keys()]} [subsys: tts, event: create_audio.inputs_prepared]"
            )
            ...
            logger.debug(
                f"Created {len(audio) / SAMPLE_RATE:.2f}s audio in {time.time() - start_t:.2f}s [subsys: tts, event: create_audio.complete]"
            )
            return audio, SAMPLE_RATE

        except Exception as e:
            logger.error(
                f"Error in _create_audio: {e} [subsys: tts, event: create_audio.error]",
                exc_info=True,
            )
            raise

    def _generate_audio_from_ids(
        self,
        phonemes: str,
        voice_embedding: np.ndarray,
        speed: float = 1.0,
    ...
    def _write_audio(
        self, audio: np.ndarray, out_path: Optional[Path], sample_rate: int
    ) -> Path:
        ...
        if not (
            ...
            or os.environ.get("TTS_SKIP_SIZE_CHECK") == "1"
        ):
            try:
            ...
            if size <= 0:
                ...
                raise TTSWriteError(
                    f"Audio file is empty or does not exist: {out_path}"
                )
